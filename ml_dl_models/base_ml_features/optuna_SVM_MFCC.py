...
MODEL_SAVE_PATH    = 'SVM_on_mfcc_optuna.pkl'
OPTUNA_STORAGE     = "sqlite:///study_SVM_mfcc.db"
LOG_TO_FILE        = 'test_results_SVM_mfcc_optuna.md'
OPTUNA_N_TRIALS    = 50
...
def extract_mfcc_features(eeg_data, sfreq=250, n_mfcc=13):
    """
    eeg_data shape: (n_samples, n_channels, n_times)
    Returns shape:  (n_samples, n_channels * n_mfcc)
    """
    n_epochs, n_channels, _ = eeg_data.shape
    features = np.zeros((n_epochs, n_channels * n_mfcc), dtype=np.float32)

    for i in tqdm(range(n_epochs), desc="Extracting MFCC"):
        feats = []
        for ch in range(n_channels):
            ...
            mfcc = librosa.feature.mfcc(
                ...
                sr=sfreq,
                n_mfcc=n_mfcc,
                hop_length=int(0.010 * sfreq),
                n_fft=int(0.025 * sfreq)
            )
...
print("Loading and extracting MFCC features...")
eeg_train, y_train, eeg_val, y_val, eeg_test, y_test = load_datasets(
    TRAIN_DATASET_PATH, VAL_DATASET_PATH, TEST_DATASET_PATH
)

X_train = extract_mfcc_features(eeg_train, sfreq=SFREQ, n_mfcc=N_MFCC)
X_val   = extract_mfcc_features(eeg_val,   sfreq=SFREQ, n_mfcc=N_MFCC)
X_test  = extract_mfcc_features(eeg_test,  sfreq=SFREQ, n_mfcc=N_MFCC)
...
def run_optuna_study(n_trials=OPTUNA_N_TRIALS):
    study = optuna.create_study(
        ...
        study_name="SVM_MFCC_Precomputed",
        load_if_exists=True,
